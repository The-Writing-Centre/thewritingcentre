...
class Cart extends db_connection {
    ...
    function add_to_cart($product_id, $ip_address, $customer_id, $quantity){
        ...
        return $this->db_query("insert into cart (p_id, ip_add, c_id, qty) values('$product_id','$ip_address', '$customer_id', '$quantity')");
    }

    // remove from cart class
    function remove_from_cart($product_id, $customer_id){
        return $this->db_query("delete from cart where p_id = '$product_id' and c_id = '$customer_id'");
    }
    ...
    function update_quantity($product_id, $customer_id, $quantity){
        return $this->db_query("update cart set qty = '$quantity' where p_id = '$product_id' and c_id = '$customer_id'");
    }
  ...
  // calculate the total amount for items in the cart
    ...
    function add_order($customer_id, $invoice_no, $order_date, $order_status){
        return $this->db_query("insert into orders (customer_id, invoice_no, order_date, order_status) values('$customer_id','$invoice_no', '$order_date', '$order_status')");
    }
    ...
    function add_payment ($amount, $customer_id, $order_id, $currency, $payment_date){
        return $this->db_query("insert into payment(amt, customer_id, order_id, currency, payment_date) values('$amount', '$customer_id', '$order_id', '$currency', '$payment_date')");
    }
